...
La spécification _ES2015_ a successivement été appelée _ECMAScript Harmony_ puis _ECMAScript 6_, souvent abbrégée _es6_
...
Les fonctions `parseInt` et `parseFloat` permettent respectivement de convertir en nombre entier et nombre flottant :
...
Il est également possible de choisir le nombre de décimales après la virgule avec la méthode `toFixed`. +
...
Il est également possible d'obtenir la date actuelle exprimée en millisecondes avec `Date.now` :
